...
class FhlAbsensi extends Model
{
    ...
    const ACTIVE_DAY = 5; // Jumat (default)
    ...
    {
        return (int) FhlConfig::getValue('active_day', self::ACTIVE_DAY);
    }
    ...
    {
        return [
            'hour' => (int) FhlConfig::getValue('end_hour', self::DEFAULT_END_HOUR),
            'minute' => (int) FhlConfig::getValue('end_minute', self::DEFAULT_END_MINUTE),
        ];
    }
    ...
    public static function getActiveDayName()
    {
        ...
        return $days[self::getActiveDay()] ?? 'Jumat';
    }
