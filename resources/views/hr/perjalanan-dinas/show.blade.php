            ...
            <div class="p-4 sm:p-6">
                ...
                <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                            <div>
                                <div class="flex flex-wrap items-center gap-3">
                                    ...
                                    @endphp
                                    <span
                                        class="px-3 py-1 text-xs font-semibold rounded-full {{ $statusColors[$perjalananDinas->status] ?? 'bg-gray-100 text-gray-800' }}">
                                        {{ ucfirst($perjalananDinas->status) }}
                                    </span>
                                </div>
                                <p class="text-sm text-gray-500 mt-1">Pengajuan:
                                    {{ $perjalananDinas->created_at->format('d/m/Y H:i') }}</p>
                            </div>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Detail Perjalanan
                                Dinas</h3>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-gray-50 rounded-lg p-4">
                                <div>
                        ...
                        <div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-4">
                                <form action="{{ route('hr.perjalanan-dinas.catatan', $perjalananDinas->id) }}"
                                    method="POST">
                                    @csrf
                                    <div class="mb-3">
                                    ...
                                    <button type="submit"
                                        class="px-4 py-2 bg-[#00a2e9] text-white rounded-lg hover:bg-[#0088c4] transition text-sm">
                                        Simpan Catatan
                                    </button>
                                </form>
                                @if ($perjalananDinas->catatan_hr)
                                    <div class="mt-3 text-sm text-gray-600 border-t border-gray-200 pt-3">
                                        ...
                                        <p class="mt-1">{{ $perjalananDinas->catatan_hr }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                        ...
                        @if ($perjalananDinas->status !== 'pending')
                            <div>
                                <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Informasi
                                    Persetujuan</h3>
                                <div class="bg-gray-50 rounded-lg p-4">
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <p class="font-medium">
                                                {{ $perjalananDinas->approver->nama_lengkap ?? 'Karyawan' }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs text-gray-500">Tanggal Disetujui</p>
                                            <p class="font-medium">
                                                {{ $perjalananDinas->approved_at ? $perjalananDinas->approved_at->format('d/m/Y H:i') : '-' }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
