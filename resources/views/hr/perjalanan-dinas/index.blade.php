    ...
    <div class="flex min-h-screen bg-gray-50">
        ...
        <div class="flex-1 transition-all duration-300 md:ml-64">
            <div class="p-4 sm:p-6 lg:p-8">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                    <div>
                        <h1 class="text-2xl sm:text-3xl font-bold text-[#161758] tracking-tight">Manajemen Perjalanan Dinas
                        </h1>
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4 mb-8">
                    ...
                    @foreach ($cards as $card)
                        <div
                            class="bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow p-4 border-l-4 {{ $card['color'] }} flex items-center justify-between">
                            <div>
                ...
                <div class="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
                                ...
                                @forelse($perjalananDinas as $item)
                                    <tr class="hover:bg-gray-50/70 transition-colors">
                                        ...
                                        <td class="px-4 py-3.5 hidden sm:table-cell">
                                            <div>
                                                <p class="text-sm font-medium text-gray-900">
                                                    {{ $item->tanggal_mulai->format('d/m/Y') }}</p>
                                                <p class="text-xs text-gray-500">s/d
                                                    {{ $item->tanggal_selesai->format('d/m/Y') }}</p>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3.5">
                                            @php
                                            ...
                                            @endphp
                                            <span
                                                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold {{ $config['bg'] }} {{ $config['text'] }}">
                                        ...
                                        <td class="px-4 py-3.5 hidden lg:table-cell max-w-[180px]">
                                            @if ($item->catatan_hr)
                                                <span class="text-xs text-gray-600"
                                                    title="{{ $item->catatan_hr }}">{{ Str::limit($item->catatan_hr, 40) }}</span>
                                            @else
                                        ...
                                        <td class="px-4 py-3.5 hidden md:table-cell">
                                            @if ($item->surat_tugas)
                                                <a href="{{ route('hr.perjalanan-dinas.download', $item->id) }}"
                                                    class="inline-flex items-center gap-1 text-[#00a2e9] hover:text-[#0088c4] text-sm font-medium transition-colors">
                                        ...
                                        <td class="px-4 py-3.5 text-right">
                                            <div class="flex items-center justify-end gap-0.5 flex-wrap">
                                                ...
                                                <a href="{{ route('hr.perjalanan-dinas.show', $item->id) }}"
                                                    title="Detail"
                                                    class="text-blue-600 hover:text-blue-800 transition-colors p-1.5 rounded-lg hover:bg-blue-50">
                                                ...
                                                @if ($item->status === 'pending')
                                                    <!-- Approve -->
                                                    <form action="{{ route('hr.perjalanan-dinas.approve', $item->id) }}"
                                                    ...
                                                    <!-- Delete -->
                                                    <form action="{{ route('hr.perjalanan-dinas.destroy', $item->id) }}"
                                                        method="POST" class="inline">
                                                        @csrf @method('DELETE')
                                                ...
                                                @if ($item->status === 'approved')
                                                    <!-- Mark Selesai -->
                                                    <form
                                                        action="{{ route('hr.perjalanan-dinas.mark-selesai', $item->id) }}"
                                                        method="POST" class="inline">
                                                        @csrf
                                    ...
                                    <tr>
                                        <td colspan="7" class="px-4 py-12 text-center">
                                            <div class="flex flex-col items-center justify-center text-gray-400">
                                                ...
                                                <p class="text-sm font-medium text-gray-500">Belum ada data perjalanan
                                                    dinas untuk filter ini.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
                    ...
                    <div class="p-4 sm:p-4">
                        {{ $perjalananDinas->withQueryString()->links() }}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="rejectModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title"
            ...
            <div class="flex items-center justify-center min-h-screen px-4 py-8">
                <div class="fixed inset-0 bg-black/40 backdrop-blur-sm transition-opacity" onclick="closeRejectModal()">
                </div>
                <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all duration-300 scale-95 opacity-0"
                    id="modalContent">
                    ...
                    <form id="rejectForm" method="POST">
                        @csrf
                        <div class="mb-4">
                            <label for="catatan_hr" class="block text-sm font-medium text-gray-700 mb-1">Catatan Penolakan
                        ...
                        <button type="submit"
                            class="w-full bg-red-500 text-white px-4 py-2.5 rounded-xl hover:bg-red-600 transition-all duration-200 font-medium shadow-sm hover:shadow-md flex items-center justify-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12" />
                            </svg>
