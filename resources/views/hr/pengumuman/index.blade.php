                            ...
                            @forelse($pengumuman as $item)
                                <tr class="border-b border-gray-200 hover:bg-[#F5F5F5]">
                                    ...
                                    {{ $item->target == 'semua'
                                        ...
                                        : ($item->target == 'hr'
                                            ? 'bg-[#27438D] text-white'
                                            : 'bg-[#FCC626] text-[#1B1B1B]') }}">
                                            {{ $item->target_label }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 text-sm">
                                        @if ($item->is_sent_to_whatsapp)
                                            <span
                                                class="px-2 py-1 rounded-full text-xs font-medium bg-[#2E7D3E] text-white">
                                                ✅ Terkirim
                                            </span>
                                        ...
                                        @else
                                            <span
                                                class="px-2 py-1 rounded-full text-xs font-medium bg-gray-300 text-gray-600">
                                                ❌ Belum
                                            </span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 text-sm">
                                        ...
                                        <div class="text-xs text-gray-500">
                                            {{ $item->creator ? $item->creator->nama_lengkap : '-' }}</div>
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="flex flex-wrap gap-1">
                                            ...
                                            @if (!$item->is_sent_to_whatsapp)
                                                <a href="{{ route('hr.pengumuman.select-contact', $item->id) }}"
                                                    class="text-[#25D366] hover:text-green-700 text-sm"
                                ...
                                <tr>
                                    <td colspan="6" class="px-4 py-8 text-center text-[#1B1B1B]">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-sm text-gray-500 mt-1">Klik tombol "Buat Pengumuman" untuk
                                                menambahkan</p>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <nav class="flex justify-center" aria-label="Pagination">
                                    <ul class="flex flex-wrap gap-1">
                                        ...
                                        @if ($pengumuman->onFirstPage())
                                            <li class="disabled">
                                                <span
                                                    class="px-3 py-2 rounded-lg bg-gray-100 text-gray-400 cursor-not-allowed text-sm">
                                                    &laquo;
                                                </span>
                                            </li>
                                        @else
                                        ...
                                        @for ($page = $start; $page <= $end; $page++)
                                            @if ($page == $pengumuman->currentPage())
                                                <li class="active">
                                                    <span
                                                        class="px-3 py-2 rounded-lg bg-[#27438D] text-white text-sm font-semibold">
                                                        {{ $page }}
                                                    </span>
                                                </li>
                                            @else
