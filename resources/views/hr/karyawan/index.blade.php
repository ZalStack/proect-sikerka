        ...
        <div class="flex-1 transition-all duration-300 md:ml-64 pt-6">
            <div class="p-3 sm:p-6">
                ...
                <div class="bg-white rounded-lg shadow-md p-4 mb-6">
                    <form action="{{ route('hr.karyawan.index') }}" method="GET"
                        ...
                        <div>
                            <select name="status_filter"
                                ...
                                <option value="active" {{ request('status_filter') === 'active' ? 'selected' : '' }}>Aktif
                                </option>
                                <option value="resigned" {{ request('status_filter') === 'resigned' ? 'selected' : '' }}>
                                    Resign</option>
                            </select>
                        </div>
                            ...
                            <table class="min-w-full">
                                ...
                                <tbody>
                                    @forelse($karyawans as $karyawan)
                                        <tr
                                            class="border-b border-gray-200 hover:bg-[#F5F5F5] {{ $karyawan->is_resigned ? 'bg-red-50' : '' }}">
                                            ...
                                            <td class="px-3 sm:px-4 py-2 sm:py-3">
                                                ...
                                                @else
                                                    <div
                                                        class="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#00a2e9] flex items-center justify-center text-white font-bold text-xs sm:text-sm">
                                                        {{ strtoupper(substr($karyawan->nama_lengkap, 0, 1)) }}
                                                    </div>
                                                @endif
                                            </td>
                                            <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm">
                                                {{ $karyawan->kode_pegawai }}</td>
                                            <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm">
                                                {{ $karyawan->nama_lengkap }}</td>
                                            <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm break-all">
                                                {{ $karyawan->email }}</td>
                                            <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm">
                                                {{ $karyawan->jabatan }}</td>
                                            <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm">
                                            ...
                                            <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm">
                                                <span
                                                    class="px-2 py-1 rounded-full text-[10px] sm:text-xs font-medium {{ $karyawan->posisi === 'hr' ? 'bg-[#27438D] text-white' : 'bg-[#00a2e9] text-white' }}">
                                                    {{ $karyawan->posisi === 'hr' ? 'HR' : 'Karyawan' }}
                                                </span>
                                            </td>
                                            <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm">
                                                <span
                                                    class="px-2 py-1 rounded-full text-[10px] sm:text-xs font-medium {{ $karyawan->status_badge }}">
                                            ...
                                            <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm">
                                                <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                                                    <a href="{{ route('hr.karyawan.show', $karyawan->id) }}"
                                                        class="text-[#00a2e9] hover:text-[#27438D] transition-colors"
                                                        title="Detail">
                                                        <!-- Icon mata (eye) -->
                                                    ...
                                                    <a href="{{ route('hr.karyawan.edit', $karyawan->id) }}"
                                                        class="text-[#FCC626] hover:text-[#e6b222] transition-colors"
                                                        title="Edit">
                                                        <!-- Icon pensil (pencil) -->
                                                        <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none"
                                                            stroke="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    <form action="{{ route('hr.karyawan.destroy', $karyawan->id) }}"
                                                        method="POST" class="inline"
                                                        onsubmit="return confirm('Apakah anda yakin ingin menghapus karyawan ini?')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                            class="text-[#ec1d1d] hover:text-red-700 transition-colors"
                                                            title="Hapus">
                                                            <!-- Icon tempat sampah (trash) -->
                                                            <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none"
                                                                stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2"
                                        ...
                                        <tr>
                                            <td colspan="10" class="px-4 py-8 text-center text-[#1B1B1B]">
                                                <div class="flex flex-col items-center">
                                                    <svg class="w-12 sm:w-16 h-12 sm:h-16 text-gray-400 mb-4"
                                                    ...
                                                    <p class="text-base sm:text-lg font-semibold">Belum ada data karyawan
                                                    </p>
                                                    <p class="text-xs sm:text-sm text-gray-500 mt-1">Klik tombol "Tambah
                                                        Karyawan" untuk menambahkan</p>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforelse
                        ...
                        @forelse($karyawans as $karyawan)
                            <div class="p-3 space-y-2 {{ $karyawan->is_resigned ? 'bg-red-50' : '' }}">
                                <div class="flex items-center gap-3">
                                    @if ($karyawan->foto_profil)
                                        <img src="{{ Storage::url($karyawan->foto_profil) }}" alt=""
                                            class="w-10 h-10 rounded-full object-cover">
                                    @else
                                        <div
                                            class="w-10 h-10 rounded-full bg-[#00a2e9] flex items-center justify-center text-white font-bold text-sm">
                                            {{ strtoupper(substr($karyawan->nama_lengkap, 0, 1)) }}
                                        </div>
                                    @endif
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm font-semibold text-[#1B1B1B] truncate">
                                            {{ $karyawan->nama_lengkap }}</p>
                                        <p class="text-[11px] text-gray-500">{{ $karyawan->kode_pegawai }} •
                                            {{ $karyawan->jabatan }}</p>
                                    </div>
                                    <span
                                        class="px-2 py-0.5 rounded-full text-[10px] font-medium {{ $karyawan->status_badge }}">
                                        {{ $karyawan->status_label }}
                                    </span>
                                </div>
                                <div class="flex items-center justify-between text-[11px] text-gray-500">
                                    ...
                                    <span
                                        class="px-2 py-0.5 rounded-full text-[10px] font-medium {{ $karyawan->posisi === 'hr' ? 'bg-[#27438D] text-white' : 'bg-[#00a2e9] text-white' }}">
                                        {{ $karyawan->posisi === 'hr' ? 'HR' : 'Karyawan' }}
                                    </span>
                                </div>
                                @if ($karyawan->is_resigned)
                                ...
                                <div class="flex flex-wrap items-center gap-x-3 gap-y-1 pt-1">
                                    <a href="{{ route('hr.karyawan.show', $karyawan->id) }}"
                                        class="text-[#00a2e9] text-xs">Detail</a>
                                    <a href="{{ route('hr.karyawan.edit', $karyawan->id) }}"
                                        class="text-[#FCC626] text-xs">Edit</a>
                                    <form action="{{ route('hr.karyawan.destroy', $karyawan->id) }}" method="POST"
                        ...
                        @empty
                            <div class="p-4 py-10 text-center text-[#1B1B1B]">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">Klik tombol "Tambah Karyawan" untuk menambahkan
                                    </p>
                                </div>
                            </div>
                        @endforelse
                                    ...
                                    <nav class="flex justify-center" aria-label="Pagination">
                                        <ul class="flex flex-wrap gap-1">
                                            @if ($karyawans->onFirstPage())
                                                <li class="disabled">
                                                    <span
                                                        class="px-2 sm:px-3 py-2 rounded-lg bg-gray-100 text-gray-400 cursor-not-allowed text-xs sm:text-sm">&laquo;</span>
                                                </li>
                                            @else
                                                <li>
                                            ...
                                            @if ($start > 1)
                                                ...
                                                @if ($start > 2)
                                                    <li class="disabled">
                                                        <span
                                                            class="px-2 sm:px-3 py-2 rounded-lg bg-gray-100 text-gray-400 cursor-not-allowed text-xs sm:text-sm">...</span>
                                                    </li>
                                                @endif
                                            @endif
                                            ...
                                            @for ($page = $start; $page <= $end; $page++)
                                                @if ($page == $karyawans->currentPage())
                                                    <li class="active">
                                                        <span
                                                            class="px-2 sm:px-3 py-2 rounded-lg bg-[#27438D] text-white text-xs sm:text-sm font-semibold">{{ $page }}</span>
                                                    </li>
                                                @else
                                                    <li>
                                            ...
                                            @else
                                                <li class="disabled">
                                                    <span
                                                        class="px-2 sm:px-3 py-2 rounded-lg bg-gray-100 text-gray-400 cursor-not-allowed text-xs sm:text-sm">&raquo;</span>
                                                </li>
                                            @endif
                                        </ul>
                                    </nav>
