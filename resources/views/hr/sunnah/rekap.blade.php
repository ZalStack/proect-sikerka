    ...
    <div class="flex-1 transition-all duration-300 md:ml-64 pt-6">
        <div class="p-3 sm:p-6">
            <div class="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold font-['Montserrat'] text-[#161758]">📊 Rekap Karyawan 7SPS</h1>
                    <p class="text-sm sm:text-base text-[#27438D]">Total poin 7 Sunnah Plus Suprasional setiap karyawan per bulan</p>
                </div>
                <div class="flex flex-wrap gap-2 w-full sm:w-auto">
                    ...
                    <a href="{{ route('hr.sunnah.rekap-divisi') }}"
                       class="flex-1 sm:flex-none text-center bg-[#2E7D3E] text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-[#1a5a2a] transition-colors duration-200 text-xs sm:text-sm">
                        🏆 Rekap Divisi
                    </a>
                </div>
            </div>

            <!-- Filter Bulan/Tahun + Sudah Mengisi -->
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6">
                <div class="bg-white rounded-lg shadow-md p-3 sm:p-4 text-center">
                    <p class="text-lg sm:text-2xl font-bold text-[#161758]">{{ $totalKaryawan }}</p>
                ...
                <div class="bg-[#00a2e9] text-white rounded-lg shadow-md p-3 sm:p-4 text-center">
                    <p class="text-lg sm:text-2xl font-bold">{{ $totalKaryawanAktif }}</p>
                        ...
                        <table class="min-w-[600px] sm:min-w-full">
                            <thead class="bg-[#F5F5F5]">
                                <tr>
                                    ...
                                    <th class="px-3 sm:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold text-[#1B1B1B] hidden lg:table-cell">Status</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr class="border-b border-gray-200 hover:bg-[#F5F5F5]">
                                    ...
                                    <td class="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm">
                                        <a href="{{ route('hr.sunnah.index', ['karyawan_id' => $row['karyawan_id'], 'start_date' => $year . '-' . $month . '-01', 'end_date' => $year . '-' . $month . '-' . date('t', strtotime($year . '-' . $month . '-01'))]) }}"
                                           class="text-[#161758] hover:text-[#00a2e9] font-medium break-words">
                                            {{ $row['nama_lengkap'] }}
                                        </a>
                                    </td>
                                    ...
                                    <td class="px-3 sm:px-4 py-2 sm:py-3 hidden lg:table-cell">
                                        @if($row['total_hari'] > 0)
                                            <span class="px-2 py-1 rounded-full text-[10px] font-medium bg-[#2E7D3E] text-white">Aktif</span>
                                        @else
                                            <span class="px-2 py-1 rounded-full text-[10px] font-medium bg-gray-300 text-gray-600">Belum</span>
                                        @endif
                                    </td>
                                </tr>
                                @empty
