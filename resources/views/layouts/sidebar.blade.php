@php
    ...
    $shortName = strlen($user->nama_lengkap ?? '') > 15
        ? substr($user->nama_lengkap ?? '', 0, 15) . '...'
        : ($user->nama_lengkap ?? 'User');
@endphp

<aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 bg-[#161758] transform transition-transform duration-300 ease-in-out -translate-x-full md:translate-x-0">
    <div class="h-full flex flex-col pt-16">
        <div class="p-4">
            <!-- User Profile Card -->
            <div class="bg-[#27438D] rounded-lg p-4 mb-6">
                <div class="flex items-center space-x-3">
                    <div class="w-12 h-12 rounded-full bg-[#00a2e9] flex items-center justify-center text-white text-xl font-bold overflow-hidden flex-shrink-0">
                        @if($userPhoto)
            ...
            <nav class="space-y-1">
                @if($isHr)
                    ...
                    <a href="https://englishtoday.read1kpmseikhlasnya.com/"
                       {{-- onclick="event.preventDefault(); alert('Menu English Today saat ini masih dalam tahap pengembangan dan akan segera tersedia.');" --}}
                       class="flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors duration-200 text-[#F5F5F5] hover:bg-[#27438D] hover:text-white">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
