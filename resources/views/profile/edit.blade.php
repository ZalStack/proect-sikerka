            ...
            <div class="p-6 bg-gradient-to-r from-[#f8faff] to-white border-b border-gray-200">
                <div class="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
                    <div class="flex-shrink-0">
                        @if($user->foto_profil)
                            <img src="{{ Storage::url($user->foto_profil) }}" alt="Foto" class="w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover border-4 border-[#00a2e9]">
                        @else
                            <div class="w-24 h-24 sm:w-32 sm:h-32 rounded-full bg-[#00a2e9] flex items-center justify-center text-white text-3xl sm:text-4xl font-bold">
                                {{ strtoupper(substr($user->nama_lengkap, 0, 1)) }}
                            </div>
                        @endif
                    </div>
                    <div class="flex-1 text-center md:text-left">
                        <h2 class="text-xl sm:text-2xl font-bold text-[#161758] break-words">{{ $user->nama_lengkap }}</h2>
                        ...
                        <div class="flex flex-wrap items-center gap-2 mt-2 justify-center md:justify-start">
                            <span class="inline-block px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-sm font-medium {{ $user->status_badge }}">
                                {{ $user->status_label }}
                            </span>
                            <span class="inline-block px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-sm font-medium {{ $user->posisi === 'hr' ? 'bg-[#27438D] text-white' : 'bg-[#00a2e9] text-white' }}">
            ...
            <div class="p-6">
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">ID Karyawan</label><p class="text-sm sm:text-base text-[#27438D] font-semibold break-words">{{ $user->kode_pegawai }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Nama Lengkap</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->nama_lengkap }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Tempat Lahir</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->tempat_lahir ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Tanggal Lahir</label><p class="text-sm sm:text-base text-[#27438D]">{{ $user->tanggal_lahir ? $user->tanggal_lahir->format('d-m-Y') : '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Jenis Kelamin</label><p class="text-sm sm:text-base text-[#27438D]">{{ $user->jenis_kelamin ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Nama Ibu Kandung</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->nama_ibu_kandung ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">NIK</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->nik ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">No KK</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->no_kk ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Agama</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->agama ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Status Pernikahan</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->status_pernikahan ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Jumlah Anak</label><p class="text-sm sm:text-base text-[#27438D]">{{ $user->jumlah_anak ?? 0 }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Email</label><p class="text-sm sm:text-base text-[#27438D] break-all">{{ $user->email }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Golongan Darah</label><p class="text-sm sm:text-base text-[#27438D]">{{ $user->golongan_darah ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">NPWP</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->npwp ?? '-' }}</p></div>
                    </div>
                </div>

                <!-- Kategori: Informasi Profesional -->
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Jabatan</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->jabatan }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Divisi</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->divisi ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Posisi</label><p class="text-sm sm:text-base text-[#27438D]"><span class="px-2 py-1 rounded-full text-xs font-medium {{ $user->posisi === 'hr' ? 'bg-[#27438D] text-white' : 'bg-[#00a2e9] text-white' }}">{{ $user->posisi === 'hr' ? 'HR' : 'Karyawan' }}</span></p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Status Karyawan</label><p class="text-sm sm:text-base text-[#27438D]"><span class="px-2 py-1 rounded-full text-xs font-medium {{ $user->status_badge }}">{{ $user->status_label }}</span></p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Tanggal Bergabung</label><p class="text-sm sm:text-base text-[#27438D]">{{ $user->tanggal_bergabung ? $user->tanggal_bergabung->format('d-m-Y') : '-' }}</p></div>
                        @if($user->end_date)
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        @if($user->is_continuing_education)
                            <div>
                                <label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Program</label>
                                <p class="text-sm sm:text-base text-[#27438D]">{{ $user->continuing_program ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Perguruan Tinggi</label>
                                <p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->continuing_perguruan_tinggi ?? '-' }}</p>
                            </div>
                            <div>
                                <label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Jurusan / Program Studi</label>
                                <p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->continuing_jurusan ?? '-' }}</p>
                            </div>
                        @endif
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Pendidikan Terakhir</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->pendidikan_terakhir ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Perguruan Tinggi</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->perguruan_tinggi ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Jurusan / Program Studi</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->jurusan ?? '-' }}</p></div>
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">IPK Terakhir</label><p class="text-sm sm:text-base text-[#27438D]">{{ $user->ipk_terakhir ?? '-' }}</p></div>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div><label class="text-xs sm:text-sm text-[#1B1B1B] font-medium">Nomor Telepon</label><p class="text-sm sm:text-base text-[#27438D] break-words">{{ $user->nomor_telepon ?? '-' }}</p></div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div>
                            ...
                            @if($user->foto_profil)
                                <img src="{{ Storage::url($user->foto_profil) }}" alt="Foto Profil" class="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover mt-1 border">
                            @else
                ...
                <div class="p-6">
                    ...
                    <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <!-- KOLOM KIRI: Pendidikan Lanjutan (selalu terlihat) -->
                            <div>
                                <div class="mb-4">
                                ...
                                <div id="continuing_fields">
                                    <div class="mb-4">
                                        <label class="block text-sm font-medium text-[#1B1B1B] mb-1">Program Pendidikan Yang Diambil</label>
            ...
            <form action="{{ route('profile.update-password') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="mb-4 relative">
                        ...
                        <input type="password" name="current_password" id="current_password" required class="w-full px-4 py-2 pr-10 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00a2e9]">
                        <button type="button" onclick="togglePassword('current_password')" class="absolute right-2 top-8 text-gray-500 hover:text-gray-700"><i class="fas fa-eye"></i></button>
                    ...
                    <div class="mb-4 relative">
                        ...
                        <input type="password" name="password" id="password" required class="w-full px-4 py-2 pr-10 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00a2e9]">
                        <button type="button" onclick="togglePassword('password')" class="absolute right-2 top-8 text-gray-500 hover:text-gray-700"><i class="fas fa-eye"></i></button>
                    ...
                    <div class="mb-4 relative">
                        ...
                        <input type="password" name="password_confirmation" id="password_confirmation" required class="w-full px-4 py-2 pr-10 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00a2e9]">
                        <button type="button" onclick="togglePassword('password_confirmation')" class="absolute right-2 top-8 text-gray-500 hover:text-gray-700"><i class="fas fa-eye"></i></button>
                        @error('password_confirmation') <p class="mt-1 text-sm text-[#ec1d1d]">{{ $message }}</p> @enderror
                    </div>
                </div>
                <div class="mt-6">
                    <button type="submit" class="bg-[#FCC626] text-[#1B1B1B] px-6 py-2 rounded-lg hover:bg-[#e6b222] transition-colors duration-200">Ubah Password</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    // Tidak ada lagi toggle untuk continuing_fields karena selalu ditampilkan
