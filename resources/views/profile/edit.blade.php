            ...
            <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <!-- ID Pegawai -->
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-[#1B1B1B] mb-1">ID Pegawai <span
                                    class="text-[#ec1d1d]">*</span></label>
                            <input type="text" name="kode_pegawai" value="{{ old('kode_pegawai', $user->kode_pegawai) }}"
                                required
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="golongan_darah"
                                ...
                                <option value="">Pilih</option>
                                <option value="A"
                                    {{ old('golongan_darah', $user->golongan_darah) === 'A' ? 'selected' : '' }}>A</option>
                                <option value="B"
                                    {{ old('golongan_darah', $user->golongan_darah) === 'B' ? 'selected' : '' }}>B</option>
                                <option value="AB"
                                    {{ old('golongan_darah', $user->golongan_darah) === 'AB' ? 'selected' : '' }}>AB
                                </option>
                                <option value="O"
                                    {{ old('golongan_darah', $user->golongan_darah) === 'O' ? 'selected' : '' }}>O</option>
                            </select>
                    ...
                    @if ($isHr)
                        ...
                        <div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="divisi"
                                    ...
                                    <option value="HRD" {{ old('divisi', $user->divisi) === 'HRD' ? 'selected' : '' }}>
                                        HRD</option>
                                    <option value="IT" {{ old('divisi', $user->divisi) === 'IT' ? 'selected' : '' }}>
                                        IT</option>
                                    <option value="KPD" {{ old('divisi', $user->divisi) === 'KPD' ? 'selected' : '' }}>
                                        KPD</option>
                                    <option value="LPS" {{ old('divisi', $user->divisi) === 'LPS' ? 'selected' : '' }}>
                                        LPS</option>
                                    <option value="MEDIA"
                                        {{ old('divisi', $user->divisi) === 'MEDIA' ? 'selected' : '' }}>MEDIA</option>
                                    <option value="PENDIDIKAN"
                                        {{ old('divisi', $user->divisi) === 'PENDIDIKAN' ? 'selected' : '' }}>PENDIDIKAN
                                    </option>
                                    <option value="PKA" {{ old('divisi', $user->divisi) === 'PKA' ? 'selected' : '' }}>
                                        PKA</option>
                                    <option value="RG" {{ old('divisi', $user->divisi) === 'RG' ? 'selected' : '' }}>
                                        RG</option>
                                    <option value="SAPRAS"
                                        {{ old('divisi', $user->divisi) === 'SAPRAS' ? 'selected' : '' }}>SAPRAS</option>
                                </select>
                                @error('divisi')
                                    <p class="mt-1 text-sm text-[#ec1d1d]">{{ $message }}</p>
                                @enderror
                        ...
                        <div>
                                ...
                                <div>
                                    ...
                                    <select name="status" required
                                        ...
                                        <option value="Internship"
                                            {{ old('status', $user->status) === 'Internship' ? 'selected' : '' }}>Magang
                                        </option>
                                    </select>
                                    @error('status')
                                        <p class="mt-1 text-sm text-[#ec1d1d]">{{ $message }}</p>
                            ...
                            <!-- Tanggal Bergabung -->
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-[#1B1B1B] mb-1">Tanggal Bergabung <span
                                        class="text-[#ec1d1d]">*</span></label>
                    ...
                    @else
                        ...
                        <div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text"
                                    value="{{ $user->tanggal_bergabung ? $user->tanggal_bergabung->format('d-m-Y') : '-' }}"
                                    disabled class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100">
                            </div>

                            <div class="mb-4">
                                <label class="block text-sm font-medium text-[#1B1B1B] mb-1">NPWP</label>
                                <input type="text" value="{{ $user->npwp ?? '-' }}" disabled
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100">
                            </div>
                        </div>
                    @endif
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="pendidikan_terakhir_new"
                                ...
                                <option value="">Pilih</option>
                                <option value="SMP"
                                    {{ old('pendidikan_terakhir_new', $user->pendidikan_terakhir_new) === 'SMP' ? 'selected' : '' }}>
                                    SMP</option>
                                <option value="SMA/MA"
                                    {{ old('pendidikan_terakhir_new', $user->pendidikan_terakhir_new) === 'SMA/MA' ? 'selected' : '' }}>
                                    SMA/MA</option>
                                <option value="SMK"
                                    {{ old('pendidikan_terakhir_new', $user->pendidikan_terakhir_new) === 'SMK' ? 'selected' : '' }}>
                                    SMK</option>
                                <option value="D1"
                                    {{ old('pendidikan_terakhir_new', $user->pendidikan_terakhir_new) === 'D1' ? 'selected' : '' }}>
                                    D1</option>
                                <option value="D2"
                                    {{ old('pendidikan_terakhir_new', $user->pendidikan_terakhir_new) === 'D2' ? 'selected' : '' }}>
                                    D2</option>
                                <option value="D3"
                                    {{ old('pendidikan_terakhir_new', $user->pendidikan_terakhir_new) === 'D3' ? 'selected' : '' }}>
                                    D3</option>
                                <option value="D4"
                                    {{ old('pendidikan_terakhir_new', $user->pendidikan_terakhir_new) === 'D4' ? 'selected' : '' }}>
                                    D4</option>
                                <option value="S1"
                                    {{ old('pendidikan_terakhir_new', $user->pendidikan_terakhir_new) === 'S1' ? 'selected' : '' }}>
                                    S1</option>
                                <option value="S2"
                                    {{ old('pendidikan_terakhir_new', $user->pendidikan_terakhir_new) === 'S2' ? 'selected' : '' }}>
                                    S2</option>
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="sedang_melanjutkan_pendidikan"
                                value="{{ old('sedang_melanjutkan_pendidikan', $user->sedang_melanjutkan_pendidikan) }}"
