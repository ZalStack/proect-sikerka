    ...
    <div class="ml-64 flex-1 p-6">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6">
            <div class="p-6">
                ...
                <!-- Informasi Detail (sama seperti sebelumnya, hanya ditampilkan) -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    ...
                    <div class="space-y-4">
                        <div><label class="text-sm text-[#1B1B1B] font-medium">ID Karyawan</label><p class="text-[#27438D] font-semibold">{{ $user->kode_pegawai }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Nama Lengkap</label><p class="text-[#27438D]">{{ $user->nama_lengkap }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Tempat Lahir</label><p class="text-[#27438D]">{{ $user->tempat_lahir ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Tanggal Lahir</label><p class="text-[#27438D]">{{ $user->tanggal_lahir ? $user->tanggal_lahir->format('d-m-Y') : '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Jenis Kelamin</label><p class="text-[#27438D]">{{ $user->jenis_kelamin ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Nama Ibu Kandung</label><p class="text-[#27438D]">{{ $user->nama_ibu_kandung ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">NIK</label><p class="text-[#27438D]">{{ $user->nik ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">No KK</label><p class="text-[#27438D]">{{ $user->no_kk ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Agama</label><p class="text-[#27438D]">{{ $user->agama ?? '-' }}</p></div>
                    </div>
                    <div class="space-y-4">
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Status Pernikahan</label><p class="text-[#27438D]">{{ $user->status_pernikahan ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Jumlah Anak</label><p class="text-[#27438D]">{{ $user->jumlah_anak ?? 0 }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Email</label><p class="text-[#27438D]">{{ $user->email }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Golongan Darah</label><p class="text-[#27438D]">{{ $user->golongan_darah ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">NPWP</label><p class="text-[#27438D]">{{ $user->npwp ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Status Karyawan</label><p class="text-[#27438D]"><span class="px-2 py-1 rounded-full text-xs font-medium {{ $user->status_badge }}">{{ $user->status_label }}</span></p></div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Jabatan</label><p class="text-[#27438D]">{{ $user->jabatan }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Divisi</label><p class="text-[#27438D]">{{ $user->divisi ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Posisi</label><p class="text-[#27438D]"><span class="px-2 py-1 rounded-full text-xs font-medium {{ $user->posisi === 'hr' ? 'bg-[#27438D] text-white' : 'bg-[#00a2e9] text-white' }}">{{ $user->posisi === 'hr' ? 'HR' : 'Karyawan' }}</span></p></div>
                    </div>
                    <div class="space-y-4">
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Tanggal Bergabung</label><p class="text-[#27438D]">{{ $user->tanggal_bergabung ? $user->tanggal_bergabung->format('d-m-Y') : '-' }}</p></div>
                        @if($user->end_date)
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Tanggal Berakhir</label><p class="text-[#27438D]">{{ $user->end_date->format('d-m-Y') }}</p></div>
                        @endif
                    </div>
                    ...
                    <div class="space-y-4">
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Sedang Melanjutkan Pendidikan</label>
                            <p class="text-[#27438D]">
                                @if($user->is_continuing_education)
                                    {{ $user->continuing_program ?? '-' }} - {{ $user->continuing_perguruan_tinggi ?? '' }} {{ $user->continuing_jurusan ? '('. $user->continuing_jurusan .')' : '' }}
                                @else
                                    Tidak
                                @endif
                            </p>
                        </div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Pendidikan Terakhir</label><p class="text-[#27438D]">{{ $user->pendidikan_terakhir ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Perguruan Tinggi</label><p class="text-[#27438D]">{{ $user->perguruan_tinggi ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Jurusan</label><p class="text-[#27438D]">{{ $user->jurusan ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">IPK Terakhir</label><p class="text-[#27438D]">{{ $user->ipk_terakhir ?? '-' }}</p></div>
                    </div>
                    <div class="space-y-4">
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Tahun Lulus</label><p class="text-[#27438D]">{{ $user->tahun_lulus ?? '-' }}</p></div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Nomor Telepon</label><p class="text-[#27438D]">{{ $user->nomor_telepon ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">No WA</label><p class="text-[#27438D]">{{ $user->no_wa ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Nama Kontak Darurat</label><p class="text-[#27438D]">{{ $user->nama_kontak_darurat ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Telepon Kontak Darurat</label><p class="text-[#27438D]">{{ $user->telepon_kontak_darurat ?? '-' }}</p></div>
                    </div>
                    <div class="space-y-4">
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Alamat</label><p class="text-[#27438D]">{{ $user->alamat ?? '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Alamat Domisili</label><p class="text-[#27438D]">{{ $user->alamat_domisili ?? '-' }}</p></div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Tanggal Pengangkatan Tetap</label><p class="text-[#27438D]">{{ $user->tanggal_pengangkatan_tetap ? $user->tanggal_pengangkatan_tetap->format('d-m-Y') : '-' }}</p></div>
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Nomor Rekening</label><p class="text-[#27438D]">{{ $user->nomor_rekening ?? '-' }}</p></div>
                    </div>
                    <div class="space-y-4">
                        <div><label class="text-sm text-[#1B1B1B] font-medium">Nama Bank</label><p class="text-[#27438D]">{{ $user->nama_bank ?? 'BSI' }}</p></div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <!-- Informasi Profesional (semua bisa diedit) -->
                        <div class="md:col-span-2">
                        ...
                        <div>
                            ...
                            <div id="continuing_fields" style="{{ old('is_continuing_education', $user->is_continuing_education) ? '' : 'display: none;' }}">
                                <div class="mb-4">
                        ...
                        <div>
                            ...
                            <div class="mb-4">
                                ...
                                @if ($user->foto_profil)
                                    <div class="mt-2"><img src="{{ Storage::url($user->foto_profil) }}" alt="Foto Profil" class="w-20 h-20 rounded-full object-cover"></div>
                                @endif
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Tidak perlu update posisi di sini karena posisi akan di-set oleh controller
        // tapi kita bisa menampilkan info, tidak wajib.
        ...
        function toggleContinuingFields() {
            let checked = false;
            radios.forEach(radio => { if (radio.checked && radio.value === '1') checked = true; });
            continuingFields.style.display = checked ? 'block' : 'none';
        }
        radios.forEach(radio => radio.addEventListener('change', toggleContinuingFields));
