    ...
    <div class="flex-1 transition-all duration-300 md:ml-64 pt-6">
        <div class="p-4 sm:p-6">
            <div class="mb-6">
                ...
                <h1 class="text-xl sm:text-2xl font-bold font-['Montserrat'] text-[#161758]">Edit Perjalanan Dinas</h1>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-lg transition-all duration-300 p-4 sm:p-6">
                <form action="{{ route('karyawan.perjalanan-dinas.update', $perjalananDinas->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        @error('agenda') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="text" name="tanggal_mulai" id="tanggal_mulai"
                                   ...
                                   value="{{ old('tanggal_mulai', $perjalananDinas->tanggal_mulai->format('Y-m-d')) }}" required placeholder="dd/mm/yyyy" autocomplete="off">
                        ...
                        <div>
                            ...
                            <input type="text" name="tanggal_selesai" id="tanggal_selesai"
                                   ...
                                   value="{{ old('tanggal_selesai', $perjalananDinas->tanggal_selesai->format('Y-m-d')) }}" required placeholder="dd/mm/yyyy" autocomplete="off">
                    ...
                    <div class="mb-6">
                        ...
                        @if($perjalananDinas->surat_tugas)
                            <p class="text-xs text-gray-500 mt-1">File saat ini: <a href="{{ route('karyawan.perjalanan-dinas.download', $perjalananDinas->id) }}" class="text-[#00a2e9] hover:underline">Download</a></p>
                        @endif
                        @error('surat_tugas') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div class="mt-6 flex flex-wrap gap-4">
                        <button type="submit" class="w-full sm:w-auto bg-[#27438D] text-white px-6 py-2 rounded-lg hover:bg-[#161758] transition font-medium text-sm sm:text-base">Simpan Perubahan</button>
                        <a href="{{ route('karyawan.perjalanan-dinas.index') }}" class="w-full sm:w-auto text-center bg-gray-500 text-white px-6 py-2 rounded-lg hover:bg-gray-600 transition-colors">Batal</a>
                    </div>
                </form>
            </div>
        </div>
